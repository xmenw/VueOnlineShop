    ...
    <div class="datail">
          ...
          <div class="list">
            ...
            <div class="cnt">
              ...
              <span class="add"
                    @click="addNum">+</span>
              <span id="kucun"
                    ref="kucun"> 库存{{ shop.num }}件</span>
            </div>
            <div class="button">
              <form action="/api/inserShopCar"
                    method="post">
                <input type="text"
                       name="id"
                       v-model="shop.id"
                ...
                <button class="shopcar"
                        @click.prevent="insertShop">加入购物车</button>
                <button class="shopcar"
                        @click.prevent="collectShop">收藏</button>
                <button class="shopcar"
                        @click.prevent="buyShop">购买</button>
              </form>
            </div>
          </div>
        </div>
      ...
      <div class="introduce">
        <ul class="ul">
          <router-link tag="li"
                       active-class="active"
                       :to="{name: 'Comment', params: {id: shop.id}}">商品评论</router-link>
          <router-link tag="li"
                       active-class="active"
                       :to="{name: 'Detail', params: {id: shop.id}}">宝贝详情</router-link>
        </ul>
      </div>
    </div>
    <keep-alive>
...
export default {
  ...
  methods: {
    ...
    buyShop () {
      ...
      this.shop.totalPrice = this.shop.count * this.shop.price;
    ...
    addLookNum (id) {
      ...
      let time = month + '-' + day;
      this.$axios.get(`/api/addBuyNum?id=${id}&&time=${time}`)
        .then((res) => {
          console.log(res)
        })
        .catch((err) => {
...
.shopDeatil {
  ...
  .cente .xinxi .list .jg {
    ...
    background-image: linear-gradient(
      to bottom,
      rgba(100, 161, 214, 0.5) 0,
      rgba(100, 161, 214, 0.1) 100%
    );
  }
