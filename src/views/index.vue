  ...
  <div id="header">
    <div class="cover">
      ...
      <ul class="right">
        ...
        <li v-for="router in $route.meta"
            :key="router.path">
          <router-link v-show="isLogin"
                       :to="{name: router.path}">{{ router.name }}</router-link>
        </li>
      </ul>
    </div>
    <!-- 搜索区 -->
        ...
        <div class="search">
          <div class="search-input">
            <form method="get">
              ...
              <div class="button">
                <button @click.prevent.enter="searchType"
...
export default {
  ...
  mounted () {
    ...
    param.append("date", `${date.getFullYear()}-${month}-${day}`);
    param.append("nums", 0);
    this.$axios.post(`/api/reportVisitNums`, param)
    setTimeout(() => {
      this.searchByTypes();
    }, 1000);
  },
  methods: {
    ...
    async searchType () {
      ...
      this.searchArr = JSON.parse(localStorage.getItem(this.user.name + 'searchs')) || []
      this.isSearch = false;
      this.searchArr.push(value)
      this.searchArr = [...new Set(this.searchArr)].slice(-3)
      localStorage.setItem(this.user.name + 'searchs', JSON.stringify(this.searchArr))
      this.searchByTypes();
    ...
    async searchByTypes () {
      ...
      params.append('types', JSON.stringify(this.searchArr))
      try {
